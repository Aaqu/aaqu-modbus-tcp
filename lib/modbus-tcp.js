...
class ModbusClient extends EventEmitter {
    ...
    _processResponse(frame) {
        ...
        if (functionCode & 0x80) {
            ...
            request.reject(new ModbusError(message, exceptionCode, Buffer.from(frame)));
            return;
        }

        const pdu = frame.slice(MBAP.HEADER_LENGTH);
        request.resolve(this._parseResponse(request.functionCode, pdu, frame));
    }

    _parseResponse(functionCode, pdu, frame) {
